...
do
	...
	ssh $vm "sudo sh -c 'echo redhat | passwd hacluster --stdin'"
...
ssh nodea.example.com "sudo sh -c 'pcs stonith create fence_node1 fence_ipmilan pcmk_host_list="nodea.example.com" ipaddr="192.168.47.220" login="admin" passwd="redhat" lanplus=1  power_wait=4; pcs stonith create fence_node2 fence_ipmilan pcmk_host_list="nodeb.example.com" ipaddr="192.168.47.221" login="admin" passwd="redhat" lanplus=1  power_wait=4; pcs stonith create fence_node3 fence_ipmilan pcmk_host_list="nodec.example.com" ipaddr="192.168.47.222" login="admin" passwd="redhat" lanplus=1  power_wait=4'"

ssh nodea.example.com "sudo sh -c 'pcs constraint location fence_node1 prefers nodea.example.com; pcs constraint location fence_node2 prefers nodeb.example.com; pcs constraint location fence_node3 prefers nodec.example.com'"
...
ssh nodea.example.com "sudo sh -c '/sbin/lvmconf --enable-cluster; systemctl disable lvm2-lvmetad --now'"

ssh nodeb.example.com "sudo sh -c '/sbin/lvmconf --enable-cluster; systemctl disable lvm2-lvmetad --now'"

ssh nodec.example.com "sudo sh -c '/sbin/lvmconf --enable-cluster; systemctl disable lvm2-lvmetad --now'"
...
ssh nodea.example.com "sudo sh -c 'pvcreate /dev/sda -ff -y; sleep 5'"
...
ssh nodea.example.com "sudo sh -c 'pcs constraint order start clusterfs-clone then web;  pcs constraint location web prefers nodea.example.com'"
